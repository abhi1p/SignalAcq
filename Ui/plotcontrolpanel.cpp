...
void PlotControlPanel::setChannelInfoModel(ChannelInfoModel* model)
{
    ...
    connect(&showAllAct, &QAction::triggered, model, [model]{model->resetVisibility(true);});
    connect(&hideAllAct, &QAction::triggered, model, [model]{model->resetVisibility(false);});
}

...
QString RecordPanel::formatTimeStamp(QString t) const
{
    ...
    time_t rawtime;
    struct tm timeinfo; // Use local structure instead of a pointer

    time(&rawtime);

    // Use localtime_s instead of localtime
    if (localtime_s(&timeinfo, &rawtime) != 0) {
        // Handle error - could not convert time
        delete[] r;
        return QString();
    }

    // Use strftime to format time into the string
    strftime(r, maxSize, t.toLatin1().data(), &timeinfo);

    // Convert C-string to QString and clean up memory
    auto rs = QString(r);
    delete[] r;
    return rs;
}

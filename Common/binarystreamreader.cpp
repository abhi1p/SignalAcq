...
BinaryStreamReader::BinaryStreamReader(AbstractDevice* device, QObject* parent) :
...
unsigned BinaryStreamReader::readData()
{
    ...
    unsigned packageSize = sampleSize * _numChannels; //(_device->*(_device->readLine))
    unsigned bytesAvailable = _device->bytesAvailable();
    // unsigned bytesAvailable = (_device->*(_device->bytesAvailable))();
    unsigned totalRead = 0;

    // skip 1 byte if requested
    if (skipByteRequested && bytesAvailable > 0)
    {
        // (_device->*(_device->read))(1);
        _device->read(1);
        totalRead++;
        skipByteRequested = false;
        bytesAvailable--;
    ...
    {
        // (_device->*(_device->read))(sampleSize);
    ...
    if (paused)
    {
        ...
        // (_device->*(_device->read))(numBytesToRead);

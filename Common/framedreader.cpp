...
FramedReader::FramedReader(AbstractDevice *device, QObject* parent) :
    AbstractReader(device, parent)
{
    ...
    connect(&_settingsWidget, &FramedReaderSettings::checksumChanged,
            this,[this](bool enabled){checksumEnabled = enabled; reset();});

    connect(&_settingsWidget, &FramedReaderSettings::debugModeChanged,
            this,[this](bool enabled){debugModeEnabled = enabled;});
...
unsigned FramedReader::readData()
{
    ...
    // while ((bytesAvailable =  (_device->*(_device->bytesAvailable))()))
    while ((bytesAvailable =  _device->bytesAvailable()))
    {
        ...
        else if (hasSizeByte && !gotSize) // skipped if fixed frame size
        {
            ...
            if (isSizeField2B)
            {
                ...
                // (_device->*(_device->read_b))((char*) &frameSize16, sizeof(frameSize16));
                _device->read_b((char*) &frameSize16, sizeof(frameSize16));
            ...
            else
            {
                ...
                // (_device->*(_device->getChar))((char*) &frameSize);
                numBytesRead++;
            }
...
void FramedReader::readFrameDataAndCheck()
{
    ...
    if (paused)
    {
        // (_device->*(_device->read))(checksumEnabled ? frameSize+1 : frameSize);
    ...
    if (checksumEnabled)
    {
        // (_device->*(_device->read_b))((char*) &rChecksum, 1);
        _device->read_b((char*) &rChecksum, 1);
        calcChecksum &= 0xFF;
...
template<typename T> double FramedReader::readSampleAs()
{
    ...
    // (_device->*(_device->read_b))((char*) &data, sizeof(data));
    _device->read_b((char*) &data, sizeof(data));

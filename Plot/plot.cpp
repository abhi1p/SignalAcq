...
Plot::Plot(QWidget* parent) :
    ...
    // QwtPlot(parent),
    // m_plotCanvas(),
    // zoomer(nullptr, false),
    // zoomer(canvas(), false),
    // sZoomer(this, &zoomer)
{
    // m_plotCanvas= new QwtPlotOpenGLCanvas();
    // setCanvas(m_plotCanvas);
    QwtPlotOpenGLCanvas *glCanvas = new QwtPlotOpenGLCanvas();
    setCanvas(glCanvas);  // Assigning the OpenGL canvas to the plot
    zoomer=new Zoomer(canvas(),false);
    sZoomer=new ScaleZoomer(this,zoomer);
    // zoomer.
    //m_plotCanvas.setPaintAttribute(QwtPlotAbstractGLCanvas::ImmediatePaint);
    // zoomer=Zoomer(canvas(), false);
    isAutoScaled = true;
    symbolSize = 0;
    numOfSamples = 1;
    plotWidth = 1;
    ...
    connect(this, &QwtPlot::itemAttached,
            this,[this](QwtPlotItem *plotItem, bool on)
            {
